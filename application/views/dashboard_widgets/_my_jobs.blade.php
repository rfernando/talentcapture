...
?>             
<div class="box box-info dashboard-box" style="border-top-color: #2e749c;">
    <div class="box-header">
        ...
        <div class="pull-right box-tools">
            ...
            <a href="{{ base_url('jobs/add_new') }}" class="btn btn-default btn-sm pull-right" data-toggle="tooltip" title="New Job" style="margin-right: 5px;">
              <i class="fa fa-briefcase"></i>
            </a>
        </div>
        <!-- /. tools -->

        <h3 class="box-title" style="font-weight: 600;font-size: 24px;">My Jobs  <a rel="popover" data-toggle="collapse" data-html="true" data-placement="right" data-parent="#accordion" data-content_id="popover_jobs" ><i class="glyphicon glyphicon-question-sign" ></i></a>
        </h3>
        <div id="popover_jobs" class="hide"> <?php $myjob = Site_messages::where('type','=','my_job')->first(); echo $myjob->msg; ?>
        </div>
    ...
    <div class="box-body"  style="overflow-y: auto; height: 492px;">
        @if($myJobs->count()>5)
            <select id="job-list-dash" name="job_lst" onChange="jobView(this.value)" style="width: 97%">
                <option value="new000">Select a Job</option>

                @foreach($myJobs->orderBy('created_at', 'desc')->get() as $myjob)
                    <option value="{{$myjob->id}}">{{$myjob->title}}</option>
                @endforeach
            </select><br><br>

            @if(isset($myjobid))
                <script>
                    document.getElementById('job-list-dash').value="{{$myjobid}}";
                </script>
            @endif

            @include('dashboard_widgets._my_job_item',['myjob' => $myjobid])
        ...
        @if(!isset($myjobid))
            @if($myJobs->count())
                <div class="box-group" id="accordion">
                    ...
                    @foreach($myJobs->orderBy('created_at', 'desc')->get() as $job)
                            ...
                            <div class="box-header">
                                <h4 class="box-title"  style="font-size: 16px;font-weight: 600;">
                                    ...
                                    <a href="{{ base_url('jobs/view_detail/'.$job->id) }}" style="color: #2e749c;" data-content_id="popover-{{ $job->id }}" rel="popover" data-html="true" data-placement="right">
                                        {{ $job->title }}
                                        <?php /*?> @if($userType != 'employer' && !$job->client_name_confidential)<?php */?>
                                ...
                                <div class="pull-right box-tools">
                                    ...
                                    @if($newCandidatesCount = $job->candidates()->where(['client_accepted'=>0])->count())
                                        <!-- <small title="New Candidate Alert" class="badge bg-red-gradient">{{ $newCandidatesCount.' New Candidate(s)' }}</small>&nbsp;&nbsp;&nbsp; --> 
                                    @endif
                                    <!--<a href="{{ base_url('jobs/close_job/'.$job->id) }}" onclick="return confirmclosed()" class="btn btn-default btn-sm pull-right" data-toggle="tooltip" title="Close Job" style="margin-right: 5px;"><i class="fa fa-lock"></i></a>-->

                                    <!--<a href="{{ base_url('jobs/view_detail/'.$job->id) }}" data-toggle="tooltip" title="View Job Details">
...
<!--																	<div class="pull-right" style="margin-top:10px;"><label class="label margin-r-5 label-danger">Rejected</label></div>-->
                        ...
                        <div class="modal fade" tabindex="-1" role="dialog" id="addPreferredDash-{{$job->id}}">
								  <div class="modal-dialog">
									  <div class="modal-content">
										  ...
										  <div class="modal-body">
										    <form action="{{ base_url('jobs/add_preferred_agencies/'.$job->id.'/dash') }}" method="post" id="addPreferredForm">
										<?php
												  	$userId = get_user('id');
            ...
            @else
                <p>You have not yet added any Jobs. <a href="{{ base_url('jobs/add_new') }}">Click here</a> to add a new Job.</p>
            @endif
        @endif
...
@section('end-script')
    ...
    <script type="application/javascript">
        ...
        $(function () {
          ...
          <?php foreach($myJobs->orderBy('created_at', 'desc')->get() as $job)
          {
            $job_id = $job->id;
          ?>
        ...
        function update_candidate_rsvp(result, element) {
            // alert(element.parent()) // for RP-760 

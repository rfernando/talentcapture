<div class="box box-danger">
    <div class="box-header with-border">
        <h3 class="box-title"><i class="fa fa-building"></i> Approved {{($agencies->count() > 2) ? 'Agencies' : 'Agency' }}</h3>
    </div>
    <!-- /.box-header -->
                ...
                @foreach($agencies as $c)
                    ...
                    <li {{ (isset($agency->id) && $agency->id == $c->id) ? 'class="active"' : '' }}>
                        ...
                        @if(isset($userType) && $userType == 'admin')
                            <a href="{{ base_url('admin/jobs/agency_detail/'.$job->id.'/'. $c->id) }}" >
                                <i class="fa fa-building-o"></i>
                                {{(isset($c->user_profile->company_name)) ? $c->first_name.' '.$c->last_name.' @ '.$c->user_profile->company_name : $candidate->agency()->first()->first_name.' '.$candidate->agency()->first()->last_name.' @ '.$candidate->agency()->first()->user_profile()->first()->company_name}}
